...
# - Next-fit, first-fit, and best-fit algorithms should be stored in separate
# files.
...
# - The submission should either be the files themselves, or a zip file not
# containing any directories.
# Note: The provided code use features from Python 3.9+, feel free to remove them
# if using an earlier version of Python.
# It is recommended for you to use Python 3.10 or Python 3.11 to test your
# code.
# Both versions are available on OpenLab, perhaps requiring a module load
# (e.g., module load python/3.10).
# Explanations for ZipZipTree public member functions:
# any variable annotated with KeyType should use the same type for each tree, and
...
# if rank is not provided, a random rank should be selected by using
# get_random_rank().
...
# creates.
# when the function returns, this should indicate the final free
# space available in each bin.
...
# As such, the assignments list for these algorithms will have little practical
# usage other than testing.
